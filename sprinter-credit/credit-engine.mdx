---
title: "Credit Engine"
description: "How the Sprinter Credit Engine handles credit line creation, drawdowns, repayments, and liquidations"
---
...
Drawing credit is not the end — it is the beginning. The real value of Sprinter Credit is what happens _after_ USDC leaves the credit line: credit drawn, USDC delivered, real-world action completed.
...
| Subscription | Service provider | Recurring payment drawn automatically via an [Operator](/sprinter-credit/policy-engine#credit-operators) |
...
```mermaid
flowchart LR
  A[User's Collateral] --> B[Sprinter Credit Engine]
  B -->|draw USDC| C[Receiver Address]
  C --> D[Card Settlement]
  C --> E[Wallet Payment]
  C --> F[Agent Execution]
  C --> G[Merchant / Protocol]
```
</div>
...
Sprinter Credit provides one unified credit line — and in the case of collateralized credit, users can deposit assets and DeFi positions across all supported chains. See [Supported Assets & Strategies](/sprinter-credit/supported-assets) for the full list of accepted collateral, earn vaults, and LTV parameters.
