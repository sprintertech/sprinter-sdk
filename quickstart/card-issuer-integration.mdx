...
The demo uses a generic `CardIssuer` interface that can be swapped for any card issuing platform. Here's the interface:
...
To integrate a real card issuer, implement this interface against their API and swap it into the demo server. The Sprinter Credit side stays the same — only the card issuer implementation changes.
...
<Card title="Card Issuer Integration Demo" icon="play" href="https://github.com/sprintertech/documentation/tree/main/card-issuer-demo-mock">
