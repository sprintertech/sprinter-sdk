...
<Heading as={"h1"} className={"openapi__heading"} children={"Get Route"}></Heading>

<MethodEndpoint method={"get"} path={"/v2/route"} context={"endpoint"}></MethodEndpoint>

:::tip
Networks need to be selected via URI. See [accessing the solve API](/Solve/solvestart/#accessing-the-solve-api) for further details.
:::


Get an executable swap route
