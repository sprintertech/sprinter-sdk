...
<Heading
  ...
  children={"Get the proposed price and execution plan for filling an intent"}
></Heading>

<MethodEndpoint method={"get"} path={"/quote"} context={"endpoint"}></MethodEndpoint>

Get the **proposed price and execution plan** for filling an intent. It includes information such as input/output amounts, fees, and gas costs and reflects the and reflects the cost, path, and feasibility of executing a swap or cross-chain action at a given moment.

Users (or integrators) evaluate quotes to determine if they're acceptable before triggering execution.
